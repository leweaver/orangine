from oe_bindings import runtime, Input, Quat, Entity
import math
...
class FirstPersonController:
    ...
    def __init__(self, camera: Entity):
        self.yaw = 0.0
        self.pitch = 0.0
        self.mouse_speed = 1.0 / 600.0
        self.camera = camera
        self.input_reader = runtime().input
    ...
    def update_camera(self):
        if not self.camera:
            return
        quat = Quat().from_rotation_y(self.yaw) * Quat().from_rotation_x(-self.pitch)
