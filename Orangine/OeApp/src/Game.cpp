...
void Game::Configure(HWND window, int dpi, int width, int height) {
  _window = window;
  ...
  m_scene->manager<IUser_interface_manager>().preInit_setUIScale(static_cast<float>(dpi) / 96.0f);
}
...
void Game::Tick() {
  ...
  // Updates the world.
  m_timer.Tick([&]() { m_scene->tick(m_timer); });
}
