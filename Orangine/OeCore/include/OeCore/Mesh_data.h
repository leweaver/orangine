...
class Mesh_data {
 public:
  explicit Mesh_data(Mesh_vertex_layout meshVertexLayout)
  ...
  const uint8_t* getIndexed(size_t index, size_t stride, size_t offset) const
  {
    ...
    if (startPos > dataSize || endPos > dataSize) {
      OE_THROW(std::runtime_error(
              "Illegal buffer access! index=" + std::to_string(index) + ", stride=" + std::to_string(stride) +
              ", offset=" + std::to_string(offset) + ", dataSize=" + std::to_string(dataSize)));
    }
...
struct Mesh_index_buffer_accessor : Mesh_buffer_accessor {
  Mesh_index_buffer_accessor(
          const std::shared_ptr<Mesh_buffer>& buffer, Element_component component, uint32_t count, uint32_t stride,
          uint32_t offset);
