...
class Primitive_mesh_data_factory {
 public:
  ...
  static std::shared_ptr<Mesh_data>
  createQuad(float width, float height, const SSE::Vector3& positionOffset);
  ...
  static std::shared_ptr<Mesh_data>
  createCone(float diameter, float height, size_t tessellation = 32);
  static std::shared_ptr<Mesh_data> createTeapot(size_t tessellation = 8);
  static std::shared_ptr<Mesh_data>
  createSphere(float radius = 0.5f, size_t tessellation = 16, bool invertNormals = false);
