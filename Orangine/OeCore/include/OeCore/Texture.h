...
using Texture_internal_id = int64_t;
...
class Texture {
 public:
  ...
  uint16_t getArraySlice() const
  {
    return _arraySlice;
  }

  Texture_internal_id getInternalId() const
  {
    return _internalId;
  }
 ...
 protected:
  Texture(Texture_internal_id internalId)
      : _dimension({0, 0})
  ...
  Texture(Texture_internal_id internalId, Vector2u dimension, uint32_t flags)
  ...
  uint16_t _arraySlice;

  Texture_internal_id _internalId;
};
