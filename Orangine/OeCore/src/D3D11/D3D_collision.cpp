...
oe::BoundingSphere oe::BoundingSphere::createFromPoints(Float3* points, int numPoints,
                                                        size_t strideBytes)
{
  DirectX::BoundingSphere dxBoundingSphere;
  DirectX::BoundingSphere::CreateFromPoints(dxBoundingSphere, numPoints,
                                            reinterpret_cast<DirectX::XMFLOAT3*>(points),
                                            strideBytes);
  return oe::BoundingSphere(LoadVector3(dxBoundingSphere.Center), dxBoundingSphere.Radius);
}

void oe::BoundingSphere::createMerged(BoundingSphere& out, const BoundingSphere& input1,
                                      const BoundingSphere& input2)
{
  DirectX::BoundingSphere bs;
  DirectX::BoundingSphere::CreateMerged(
      bs, DirectX::BoundingSphere(StoreVector3(input1.center), input1.radius),
...
{
  DirectX::BoundingOrientedBox bob(StoreVector3(center), StoreVector3(extents),
                                   StoreQuat(orientation));
  DirectX::BoundingSphere bs(StoreVector3(boundingSphere.center), boundingSphere.radius);
  return bob.Contains(bs);
}

void oe::BoundingFrustumRH::CreateFromMatrix(BoundingFrustumRH& Out, const SSE::Matrix4& projection)
{
  using namespace DirectX;
...
bool oe::intersect_ray_sphere(const oe::Ray& ray, const oe::BoundingSphere& sphere,
