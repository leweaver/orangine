...
const std::array<const std::map<int, std::set<int>>*, 4> g_animTypeToAllowedAccessorTypes = {
        &g_anim_translation_allowedAccessorTypes, &g_anim_rotation_allowedAccessorTypes,
        &g_anim_scale_allowedAccessorTypes, &g_anim_morph_allowedAccessorTypes};
static_assert(g_animTypeToAllowedAccessorTypes.size() == static_cast<unsigned>(Animation_type::Num_animation_type));
...
const std::map<int, Element_type> g_gltfType_elementType = {
        {TINYGLTF_TYPE_VEC2, Element_type::Vector2},  {TINYGLTF_TYPE_VEC3, Element_type::Vector3},
        {TINYGLTF_TYPE_VEC4, Element_type::Vector4},  {TINYGLTF_TYPE_MAT2, Element_type::Matrix2},
        {TINYGLTF_TYPE_MAT3, Element_type::Matrix3},  {TINYGLTF_TYPE_MAT4, Element_type::Matrix4},
...
const auto g_createSimpleMeshAccessor = [](shared_ptr<Mesh_buffer> meshBuffer, Element_type type,
                                           Element_component component, size_t count, size_t stride, size_t offset) {
  return make_unique<Mesh_buffer_accessor>(
          meshBuffer, static_cast<uint32_t>(count), static_cast<uint32_t>(stride), static_cast<uint32_t>(offset));
};

std::function<std::unique_ptr<Mesh_vertex_buffer_accessor>(
        shared_ptr<Mesh_buffer> meshBuffer, Element_type type, Element_component component, size_t count, size_t stride,
        size_t offset)>
vertexAccessorFactory(Vertex_attribute_semantic vertexAttribute)
{
  return [vertexAttribute](
                 shared_ptr<Mesh_buffer> meshBuffer, Element_type type, Element_component component, size_t count,
                 size_t stride, size_t offset) {
    return make_unique<Mesh_vertex_buffer_accessor>(
            meshBuffer, Vertex_attribute_element{vertexAttribute, type, component}, static_cast<uint32_t>(count),
            static_cast<uint32_t>(stride), static_cast<uint32_t>(offset));
  };
}

struct Loader_data {
  Loader_data(
          Model& model, string&& baseDir, IWICImagingFactory* imagingFactory, IScene_graph_manager& sceneGraphManager,
          IEntity_repository& entityRepository, IMaterial_manager& materialManager, ITexture_manager& textureManager,
          IComponent_factory& componentFactory, bool calculateBounds)
      : model(model)
...
unique_ptr<TMesh_buffer_accessor> useOrCreateBufferForAccessor(
        size_t accessorIndex, Loader_data& loaderData, const std::map<int, std::set<int>> allowedAccessorTypes,
        int expectedBufferViewTarget,
        std::function<unique_ptr<TMesh_buffer_accessor>(
                shared_ptr<Mesh_buffer>, Element_type type, Element_component component, size_t count, size_t stride,
                size_t offset)>
                accessorFactory)
{
  ...
  if (!allowedAccessorTypePos->second.empty() &&
      allowedAccessorTypePos->second.find(accessor.componentType) == allowedAccessorTypePos->second.end())
  {
  ...
  if (sourceElementSize <= 0) {
    OE_THROW(domain_error(
            "missing or unsupported accessor field: "s + (numComponentsInType == -1 ? "type" : "componentType")));
  }
  ...
  if (sourceStride == -1) OE_THROW(std::domain_error("Invalid glTF parameters for byte stride"));
  if (sourceStride == 0) sourceStride = sourceElementSize;
  ...
  if (pos == loaderData.accessorIdxToMeshBuffers.end()) {
        ...
        (elementComponent == Element_component::Unsigned_byte || elementComponent == Element_component::Signed_byte))
    {
      ...
      const auto convertedIndexAccessor = mesh_utils::create_index_buffer(
              buffer.data, static_cast<uint32_t>(accessor.count), elementComponent, static_cast<uint32_t>(sourceStride),
      ...
      meshBuffer = mesh_utils::create_buffer(
              static_cast<uint32_t>(sourceElementSize), static_cast<uint32_t>(accessor.count), buffer.data,
              static_cast<uint32_t>(sourceStride), static_cast<uint32_t>(bufferOffset));
    }
...
std::vector<SSE::Matrix4> createMatrix4ArrayFromAccessor(Loader_data& loaderData, int accessorIndex)
{
  ...
  const auto matricesAccessor = useOrCreateBufferForAccessor<Mesh_buffer_accessor>(
          accessorIndex, loaderData, {{TINYGLTF_TYPE_MAT4, {TINYGLTF_COMPONENT_TYPE_FLOAT}}}, 0,
...
vector<shared_ptr<Entity>> Entity_graph_loader_gltf::loadFile(
        string_view filePath, IScene_graph_manager& sceneGraphManager, IEntity_repository& entityRepository,
        IComponent_factory& componentFactory, bool calculateBounds) const
{
  ...
  const auto ret = loader.LoadASCIIFromString(
          &model, &err, &warn, gltfAscii.c_str(), static_cast<unsigned>(gltfAscii.length()), baseDir);
  if (!err.empty()) {
  ...
  for (size_t idx = 0; idx < loaderData.nodeIdxToEntity.size(); ++idx) {
    ...
    if (skinIdx >= numSkins) {
      OE_THROW(std::domain_error("Node[" + to_string(idx) + "] points to invalid skin index: " + to_string(skinIdx)));
    }
    ...
    try {
      ...
      for (const auto& childEntity : entity->children()) {
        if (!childEntity->getFirstComponentOfType<Mesh_data_component>()) continue;
...
bool try_create_texture(
        const Loader_data& loaderData, const tinygltf::Material& gltfMaterial, const std::string& textureName,
        oe::Material::Shader_texture_input& output)
{
  ...
  if (paramPos != gltfMaterial.values.end()) {
    ...
    if (paramPos == gltfMaterial.additionalValues.end()) return false;
  ...
  if (gltfTextureIndex < 0 || gltfTextureIndex >= static_cast<int>(loaderData.model.textures.size()))
    OE_THROW(domain_error("invalid texture index '" + to_string(gltfTextureIndex) + "' for " + textureName));
  ...
  if (gltfTexture.sampler >= 0) {
    const auto& gltfSampler = loaderData.model.samplers.at(gltfTexture.sampler);
    {
      ...
      if (!validWrap) {
        OE_THROW(domain_error("image " + to_string(gltfTexture.source) + " invalid sample wrap value"));
      }
    }

    {
      const auto validFilterType = tryParseFilterType(gltfSampler.minFilter, samplerDescriptor.minFilter) &&
                                   tryParseFilterType(gltfSampler.magFilter, samplerDescriptor.magFilter);

      if (!validFilterType) {
        OE_THROW(domain_error("image " + to_string(gltfTexture.source) + " invalid sample filter value"));
      }
    }
  }
  ...
  if (!gltfImage.uri.empty()) {
    ...
    output = {loaderData.textureManager.createTextureFromFile(filename), samplerDescriptor};
    return true;
  }
  if (!gltfImage.mimeType.empty()) {
    OE_THROW(runtime_error("mime type images are not implemented"));
  }
  OE_THROW(domain_error("image " + to_string(gltfTexture.source) + "must have either a uri or mimeType"));
  return false;
}

shared_ptr<oe::Material> create_material(const Primitive& prim, Loader_data& loaderData)
{
  ...
  if (prim.material >= loaderData.model.materials.size()) {
    OE_THROW(domain_error("refers to material index " + to_string(prim.material) + " which doesn't exist."));
  }

  const auto& gltfMaterial = loaderData.model.materials[prim.material];
  const auto withParam = [gltfMaterial](
                                 const string& name, std::function<void(const string&, const Parameter&)> found,
  ...
  const auto withScalarParam = [&withParam,
    ...
    withParam(
            ...
            [&setter](auto name, auto param) {
              if (!param.has_number_value)
                OE_THROW(domain_error("Failed to parse glTF: expected material " + name + " to be scalar"));
              setter(static_cast<float>(param.number_value));
            },
            [&setter, defaultValue](auto) { setter(defaultValue); });
  };

  const auto withColorParam = [&withParam,
    ...
    withParam(
            ...
            [&setter](auto name, auto param) {
              if (param.number_array.size() == 3) {
                setter(
                        Color(static_cast<float>(param.number_array[0]), static_cast<float>(param.number_array[1]),
                              static_cast<float>(param.number_array[2]), 1.0f));
              }
              else if (param.number_array.size() == 4) {
                setter(
                        Color(static_cast<float>(param.number_array[0]), static_cast<float>(param.number_array[1]),
                              static_cast<float>(param.number_array[2]), static_cast<float>(param.number_array[3])));
              }
              else
                OE_THROW(domain_error(
                        "Failed to parse glTF: expected material " + name + " to be a 3 or 4 element array"));
            },
            [&setter, defaultValue](auto) { setter(defaultValue); });
  };
          ...
          [&material](const string& name, const Parameter& param) {
            if (param.string_value == g_pbrPropertyValue_alphaMode_mask)
              ...
              OE_THROW(domain_error(
                      ...
                      " to be one of:" + g_pbrPropertyValue_alphaMode_mask + ", " + g_pbrPropertyValue_alphaMode_blend +
  ...
  // PBR Textures
  oe::Material::Shader_texture_input textureInput;
  if (try_create_texture(loaderData, gltfMaterial, "baseColorTexture", textureInput)) {
    material->setBaseColorTexture(textureInput);
  }
  if (try_create_texture(loaderData, gltfMaterial, "metallicRoughnessTexture", textureInput)) {
    material->setMetallicRoughnessTexture(textureInput);
  }
  if (try_create_texture(loaderData, gltfMaterial, "normalTexture", textureInput)) {
    material->setNormalTexture(textureInput);
  }
  if (try_create_texture(loaderData, gltfMaterial, "occlusionTexture", textureInput)) {
    material->setOcclusionTexture(textureInput);
  }
  if (try_create_texture(loaderData, gltfMaterial, "emissiveTexture", textureInput)) {
    material->setEmissiveTexture(textureInput);
  }
...
bool loadJointsWeights(int index, const Primitive& prim, Loader_data& loaderData, Mesh_data& meshData)
{
  ...
  if (animJointAttrPos != prim.attributes.end() && animWeightsAttrPos != prim.attributes.end()) {
    const auto loadAccessor = [&loaderData, &meshData](
                                      ...
                                      decltype(prim.attributes.find(string())) pos, const string& attrName) {
      try {
        // Animation Joints
...
shared_ptr<Entity> create_entity(vector<Node>::size_type nodeIdx, Loader_data& loaderData)
{
  ...
  auto rootEntity =
          loaderData.entityRepository.instantiate(node.name, loaderData.sceneGraphManager, loaderData.componentFactory);
  ...
  if (node.mesh > -1) {
    ...
    for (size_t primIdx = 0; primIdx < primitiveCount; ++primIdx) {
      ...
      auto primitiveEntity = loaderData.entityRepository.instantiate(
              primitiveName, loaderData.sceneGraphManager, loaderData.componentFactory);
      ...
      for (const auto& attr : prim.attributes) {
        ...
        if (accessorComponentTypePos == g_gltfComponent_elementComponent.end()) {
          OE_THROW(std::domain_error(
                  std::string("Unsupported gltf accessor component type: ") + to_string(accessor.componentType)));
        }

        meshLayoutAttributes.push_back(
                Vertex_attribute_element{vaPos->second, accessorTypePos->second, accessorComponentTypePos->second});
      }
      ...
      try {
        ...
        try {
          ...
          auto indexBufferAccessor = useOrCreateBufferForAccessor<Mesh_index_buffer_accessor>(
                  prim.indices, loaderData, g_index_allowedAccessorTypes, TINYGLTF_TARGET_ELEMENT_ARRAY_BUFFER,
                  [](shared_ptr<Mesh_buffer> meshBuffer, Element_type, Element_component component, size_t count,
                     size_t stride, size_t offset) {
                    return make_unique<Mesh_index_buffer_accessor>(
                            meshBuffer, component, static_cast<uint32_t>(count), static_cast<uint32_t>(stride),
        ...
        for (const auto& attr : prim.attributes) {
          ...
          try {
            ...
            auto vertexBufferAccessor = useOrCreateBufferForAccessor<Mesh_vertex_buffer_accessor>(
                    attr.second, loaderData, g_vertex_allowedAccessorTypes, TINYGLTF_TARGET_ARRAY_BUFFER,
            ...
            OE_THROW(std::domain_error(
                    "Error in attribute " + Vertex_attribute_meta::vsInputName(vertexAttribute) + ": " + e.what()));
          }
        }
        ...
        if (loaderData.calculateBounds) {
          ...
          auto boundingSphere = oe::BoundingSphere::createFromPoints(
                  reinterpret_cast<Float3*>(vertexBufferAccessor->buffer->data), vertexBufferAccessor->count,
        ...
        if (!prim.targets.empty()) {
          ...
          if (morphWeights.empty()) morphWeights = mesh.weights;
          if (morphWeights.empty()) morphWeights.resize(prim.targets.size(), 0.0);
          ...
          for (const auto& target : prim.targets) {
            if (targetSize != target.size()) OE_THROW(std::domain_error("Size of each target must be the same."));
            ...
            for (const auto& targetAttributeAccessor : target) {
              ...
              auto accessor = useOrCreateBufferForAccessor<Mesh_vertex_buffer_accessor>(
                      targetAttributeAccessor.second, loaderData, g_morphTarget_allowedAccessorTypes, 0,
                      vertexAccessorFactory(vertexAttribute));
        ...
        OE_THROW(std::domain_error(string("Primitive[") + to_string(primIdx) + "] is malformed. (" + e.what() + ")"));
      }
    }
  }
...
void create_animation(int animIdx, Loader_data loaderData)
{
  ...
  if (gltfAnimation.channels.empty()) return;

  std::map<int, std::shared_ptr<std::vector<float>>> samplerInputToKeyframeTimes;
  const auto populateSamplerInputKeyframeTimes = [&gltfAnimation,
                                                  &loaderData](int accessorIdx, std::vector<float>& keyframeTimes) {
    // Animation sampler
    const auto accessor = useOrCreateBufferForAccessor<Mesh_buffer_accessor>(
            accessorIdx, loaderData, {{TINYGLTF_TYPE_SCALAR, {TINYGLTF_COMPONENT_TYPE_FLOAT}}}, 0,
            g_createSimpleMeshAccessor);
    keyframeTimes.clear();
  ...
  for (size_t channelIdx = 0; channelIdx < gltfAnimation.channels.size(); ++channelIdx) {
    ...
    if (channel.target_node >= numNodes)
      OE_THROW(std::domain_error(
              "Invalid animation channel at index " + to_string(channelIdx) + " - references unknown node " +
              to_string(channel.target_node)));
    ...
    if (interpolationTypePos == g_animationSamplerInterpolationToTypeMap.end())
      OE_THROW(std::domain_error("Unknown animation sampler interpolation type: " + sampler.interpolation));
    ...
    if (samplerInputToKeyframeTimesPos == samplerInputToKeyframeTimes.end()) {
      try {
        ...
        OE_THROW(std::domain_error("Failed to load sampler " + to_string(channel.sampler) + ". " + ex.what()));
      };
    }
    else {
    ...
    // Keyframe Values
    const auto allowedAccessorTypes = g_animTypeToAllowedAccessorTypes[static_cast<unsigned>(animationType)];
    ...
    if (interpolationType == Animation_interpolation::Cubic_spline) valuesPerKeyFrame *= 3;
