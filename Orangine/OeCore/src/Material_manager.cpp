...
IMaterial_manager* oe::create_manager(
  ...
  return new Material_manager(scene, deviceRepository);
}

Material_manager::Material_manager(Scene& scene, std::shared_ptr<internal::Device_repository> deviceRepository)
...
void Material_manager::createDeviceDependentResources() {
...
void Material_manager::createPixelShader(
  ...
  compiledMaterial.pixelShader->SetPrivateData(
  ...
  if (rebuildConfig) {
    ...
    if (requiresRecompile) {
      ...
      try {
        ...
        createVertexShader(
            _rendererFeatures.enableShaderOptimization, *compiledMaterial, *material);
    ...
    if (materialContext.shaderResourceViews.size() < shaderResources.textures.size()) {
      ...
      LOG(G3LOG_DEBUG) << "Created " << material->materialType()
                       << " shader resources. Texture Count: "
                       << std::to_string(materialContext.shaderResourceViews.size());
    }
    ...
    if (materialContext.samplerStates.size() < shaderResources.samplerDescriptors.size()) {
      ...
      LOG(G3LOG_DEBUG) << "Created " << material->materialType() << " sampler states. Count: "
                       << std::to_string(materialContext.samplerStates.size());
    }
  }
...
void Material_manager::render(
  ...
  if (materialConstants->pixelConstantBuffer != nullptr) {
    ...
    ID3D11Buffer* pixelConstantBuffers[] = {materialConstants->pixelConstantBuffer->d3dBuffer,
                                            _boundLightDataConstantBuffer.Get()};
