...
size_t Vertex_attribute_meta::numComponents(Vertex_attribute attribute) {
  ...
  default:
    OE_THROW(std::logic_error("Cannot determine element size for VertexAttribute: "s.append(
        vertexAttributeToString(attribute))));
  }
}
...
Mesh_buffer::Mesh_buffer(size_t size) : data(nullptr), dataSize(size) {
...
Mesh_buffer_accessor::Mesh_buffer_accessor(
    ...
    uint32_t stride,
    uint32_t offset)
    : buffer(std::move(buffer)), count(count), stride(stride), offset(offset) {}
...
Mesh_vertex_buffer_accessor::Mesh_vertex_buffer_accessor(
    ...
    Vertex_attribute_element attributeElement,
    uint32_t count,
    uint32_t stride,
    uint32_t offset)
    : Mesh_buffer_accessor(buffer, count, stride, offset), attributeElement(attributeElement) {}

Mesh_index_buffer_accessor::Mesh_index_buffer_accessor(
    const std::shared_ptr<Mesh_buffer>& buffer,
    Element_component component,
    uint32_t count,
    uint32_t stride,
    uint32_t offset)
    : Mesh_buffer_accessor(buffer, count, stride, offset), component(component) {}
