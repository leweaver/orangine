...
class Shadow_map_texture_array_texture : public Texture {
 public:
  Shadow_map_texture_array_texture(ID3D11ShaderResourceView* srv) : Texture(srv) {}
...
Shadow_map_texture_pool::~Shadow_map_texture_pool() {
  if (_shadowMapArrayTexture2D) {
    LOG(WARNING) << "Shadow_map_texture_pool: Should call destroyDeviceDependentResources prior to "
                    "object deletion";
...
void Shadow_map_texture_pool::createDeviceDependentResources() {
  ...
  _shadowMapDepthArrayTexture =
      std::make_shared<Shadow_map_texture_array_texture>(depthShaderResourceView.Get());
  _shadowMapStencilArrayTexture =
      std::make_shared<Shadow_map_texture_array_texture>(stencilShaderResourceView.Get());
  auto arrayTextureRetriever = [this, depthShaderResourceView]() {
    return Shadow_map_texture_array_slice::Array_texture{_shadowMapArrayTexture2D.Get(),
                                                         depthShaderResourceView.Get(),
                                                         _textureArraySize};
  };
  D3D11_VIEWPORT viewport =
      CD3D11_VIEWPORT(0.0f, 0.0f, static_cast<float>(_dimension), static_cast<float>(_dimension));

  for (uint32_t slice = 0; slice < _textureArraySize; ++slice) {
    auto shadowMap = std::make_unique<Shadow_map_texture_array_slice>(
        viewport, _textureArraySize - slice - 1, _dimension, arrayTextureRetriever);
