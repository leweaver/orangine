...
std::set<std::string> Unlit_material::configFlags(
    ...
    const Mesh_vertex_layout& meshVertexLayout) const
{
    ...
    const auto hasColors = std::any_of(
        vertexLayout.begin(),
        vertexLayout.end(),
        [](const auto& vae) { return vae.semantic == Vertex_attribute_semantic{ Vertex_attribute::Color, 0 }; });
    if (hasColors) {
...
std::vector<Vertex_attribute_element> Unlit_material::vertexInputs(const std::set<std::string>& flags) const
{
    auto vertexAttributes = Base_type::vertexInputs(flags);

    if (flags.find(g_flag_enableVertexColor) != flags.end()) {
        vertexAttributes.push_back({ { Vertex_attribute::Color, 0 }, Element_type::Vector4, Element_component::Float });
    }

    return vertexAttributes;
}

Material::Shader_compile_settings Unlit_material::vertexShaderSettings(const std::set<std::string>& flags) const
{
