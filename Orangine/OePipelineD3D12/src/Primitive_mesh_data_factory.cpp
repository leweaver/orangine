...
std::string Primitive_mesh_data_factory::_name;

const Vertex_attribute_element kVertexElementPosition0 =
        Vertex_attribute_element({Vertex_attribute::Position, 0}, Element_type::Vector3, Element_component ::Float);
const Vertex_attribute_element kVertexElementColor0 =
        Vertex_attribute_element({Vertex_attribute::Color, 0}, Element_type::Vector3, Element_component ::Float);

template<> void oe::create_manager(Manager_instance<IPrimitive_mesh_data_factory>& out)
{
  out = Manager_instance<IPrimitive_mesh_data_factory>(std::make_unique<Primitive_mesh_data_factory>());
}

struct Mesh_data_builder
{
  using VertexBufferAccessorMap =
          std::unordered_map<Vertex_attribute_semantic, std::unique_ptr<Mesh_vertex_buffer_accessor>>;

  std::vector<Vertex_attribute_element> layout;
  VertexBufferAccessorMap attrSemanticToAccessor;

  std::shared_ptr<Mesh_data> buildIndexed(const std::vector<uint16_t>& indices)
  {
    auto meshData =
            std::make_shared<Mesh_data>(Mesh_vertex_layout{this->layout, Mesh_index_type::Triangles});
    meshData->vertexBufferAccessors = std::move(this->attrSemanticToAccessor);
    ...
    meshData->indexBufferAccessor = std::make_unique<Mesh_index_buffer_accessor>(
            meshBuffer, Element_component::Unsigned_short, static_cast<uint32_t>(indices.size()),
            static_cast<uint32_t>(sizeof(uint16_t)), 0);

    return meshData;
  }

  template<class TVertex_type>
  void addBufferAccessor(
          std::shared_ptr<Mesh_buffer> meshBuffer,
          const std::vector<TVertex_type>& vertices,
          Vertex_attribute_element& vertexAttributeElement,
          size_t offset)
  {
    constexpr auto vertexSize = sizeof(TVertex_type);
    auto vertexBufferAccessor = std::make_unique<Mesh_vertex_buffer_accessor>(
            meshBuffer, vertexAttributeElement, static_cast<uint32_t>(vertices.size()), static_cast<uint32_t>(vertexSize),
            static_cast<uint32_t>(offset));
    auto insertRes = attrSemanticToAccessor.insert(std::make_pair(vertexAttributeElement.semantic, std::move(vertexBufferAccessor)));
    OE_CHECK(insertRes.second);
    layout.push_back(vertexAttributeElement);
  }

  void addBufferAccessor(Vertex_attribute_element vertexAttributeElement, std::unique_ptr<Mesh_vertex_buffer_accessor> vertexBufferAccessor)
  {
    auto insertRes = attrSemanticToAccessor.insert(std::make_pair(vertexAttributeElement.semantic, std::move(vertexBufferAccessor)));
    OE_CHECK(insertRes.second);
    layout.push_back(vertexAttributeElement);
    ...
    template<class TVertex_type>
  void addPositionBufferAccessor(
          std::shared_ptr<Mesh_buffer> meshBuffer,
          const std::vector<TVertex_type>& vertices,
          uint8_t semanticIndex = 0)
  {
    auto vertexAttributeElement = Vertex_attribute_element{
            {Vertex_attribute::Position, semanticIndex},
            Element_type::Vector3,
            Element_component::Float};
    addBufferAccessor(meshBuffer, vertices, vertexAttributeElement, offsetof(TVertex_type, position));
  }

    template<class TVertex_type>
  void addColorBufferAccessor(
          std::shared_ptr<Mesh_buffer> meshBuffer,
          const std::vector<TVertex_type>& vertices,
          uint8_t semanticIndex = 0)
  {
    auto vertexAttributeElement = Vertex_attribute_element{
            {Vertex_attribute::Color, semanticIndex},
            Element_type::Vector4,
            Element_component::Float};
    addBufferAccessor(meshBuffer, vertices, vertexAttributeElement, offsetof(TVertex_type, color));
  }

    template<class TVertex_type>
  void addNormalBufferAccessor(
          std::shared_ptr<Mesh_buffer> meshBuffer,
          const std::vector<TVertex_type>& vertices,
          uint8_t semanticIndex = 0)
  {
    auto vertexAttributeElement = Vertex_attribute_element{
            {Vertex_attribute::Normal, semanticIndex},
            Element_type::Vector3,
            Element_component::Float};
    addBufferAccessor(meshBuffer, vertices, vertexAttributeElement, offsetof(TVertex_type, normal));
  }

    template<class TVertex_type>
  void addTextureBufferAccessor(
          std::shared_ptr<Mesh_buffer> meshBuffer,
          const std::vector<TVertex_type>& vertices,
          uint8_t semanticIndex = 0)
  {
    auto vertexAttributeElement = Vertex_attribute_element{
            {Vertex_attribute::Tex_coord, semanticIndex},
            Element_type::Vector2,
            Element_component::Float};
    addBufferAccessor(meshBuffer, vertices, vertexAttributeElement, offsetof(TVertex_type, textureCoordinate));
  }
};

std::shared_ptr<Mesh_data>
createMeshData(const std::vector<DirectX::VertexPosition>& vertices, const std::vector<uint16_t>& indices)
{
  ...
  Mesh_data_builder builder;
  // Position
  builder.addPositionBufferAccessor(posNormalTexBuffer, vertices);
  return builder.buildIndexed(indices);
}

std::shared_ptr<Mesh_data>
createMeshData(const std::vector<DirectX::VertexPositionColor>& vertices, const std::vector<uint16_t>& indices)
{
  ...
  Mesh_data_builder builder;
  // Position
  builder.addPositionBufferAccessor(meshBuffer, vertices);

  // Color
  builder.addColorBufferAccessor(meshBuffer, vertices);

  return builder.buildIndexed(indices);
}

std::shared_ptr<Mesh_data>
createMeshData(const std::vector<DirectX::VertexPositionNormalTexture>& vertices, const std::vector<uint16_t>& indices, bool addTangents)
{
  ...
  // Normal
  builder.addNormalBufferAccessor(posNormalTexBuffer, vertices);

  // Texcoord
  builder.addTextureBufferAccessor(posNormalTexBuffer, vertices);

  // Tangents in a separate buffer
  if (addTangents) {
    constexpr auto tangentElementSize = sizeof(XMFLOAT4);
    auto tangentsBuffer = std::make_shared<Mesh_buffer>(static_cast<int>(tangentElementSize * vertices.size()));

    auto vertexAttributeElement = Vertex_attribute_element{
            {Vertex_attribute::Tangent, 0},
            Element_type::Vector4,
            Element_component::Float};

    auto vertexBufferAccessor = std::make_unique<Mesh_vertex_buffer_accessor>(
            tangentsBuffer, vertexAttributeElement, static_cast<uint32_t>(vertices.size()), static_cast<uint32_t>(tangentElementSize),
            0);

    builder.addBufferAccessor(vertexAttributeElement, std::move(vertexBufferAccessor));
  }
...
std::shared_ptr<Mesh_data>
Primitive_mesh_data_factory::createQuad(float width, float height, const SSE::Vector3& positionOffset)
{
  const auto top = positionOffset.getY() + height, right = positionOffset.getX() + width,
             bottom = positionOffset.getY() + 0.0f, left = positionOffset.getX() + 0.0f;
  ...
  auto md = createMeshData(vertices, indices, true);
  generateTangents(md);
  return md;
}
...
std::shared_ptr<Mesh_data>
Primitive_mesh_data_factory::createSphere(float radius, size_t tessellation, bool invertNormals)
{
  std::vector<DirectX::GeometricPrimitive::VertexType> vertices;
  std::vector<uint16_t> indices;
...
std::shared_ptr<Mesh_data> Primitive_mesh_data_factory::createFrustumLines(const BoundingFrustumRH& frustum)
{
  ...
  auto md = createMeshData(vertices, indices);
...
std::shared_ptr<Mesh_data> Primitive_mesh_data_factory::createAxisWidgetLines()
{
  ...
  std::vector<DirectX::VertexPositionColor> vertices = {
          {{0, 0, 0}, StoreVector4(oe::Colors::Red)},   {{1, 0, 0}, StoreVector4(oe::Colors::Red)},
          {{0, 0, 0}, StoreVector4(oe::Colors::Green)}, {{0, 1, 0}, StoreVector4(oe::Colors::Green)},
          {{0, 0, 0}, StoreVector4(oe::Colors::Blue)},  {{0, 0, -1}, StoreVector4(oe::Colors::Blue)}};
...
void Primitive_mesh_data_factory::generateNormals(
        ...
        const Mesh_vertex_buffer_accessor& positionBufferAccessor, Mesh_vertex_buffer_accessor& normalBufferAccessor)
{
  ...
  const auto normalBufferEnd =
          normalBufferStart + normalBufferAccessor.count * normalBufferAccessor.stride + sizeof(XMFLOAT3);
    ...
    for (uint32_t idx = 0; idx < indexBufferAccessor.count;) {
      ...
      if (i0 > numVertices || i1 > numVertices || i2 > numVertices) {
        OE_THROW(std::logic_error(
                "IndexBuffer index near " + std::to_string(idx - 3) +
                " out of range of given vertexBuffer (numVertices=" + std::to_string(numVertices) + ")"));
      }

      const auto& p0 =
              LoadVector3(*reinterpret_cast<const XMFLOAT3*>(positionBufferStart + i0 * positionBufferAccessor.stride));
      const auto& p1 =
              LoadVector3(*reinterpret_cast<const XMFLOAT3*>(positionBufferStart + i1 * positionBufferAccessor.stride));
      const auto& p2 =
              LoadVector3(*reinterpret_cast<const XMFLOAT3*>(positionBufferStart + i2 * positionBufferAccessor.stride));
...
void Primitive_mesh_data_factory::initStatics() { _name = "Primitive_mesh_data_factory"; }
